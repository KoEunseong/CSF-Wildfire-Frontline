...
def forward_logits(model, imgs, args):
    logits = model(imgs)
...
def train(args):
    ...
    print(f"ARCH=deeplabv3plus | ENCODER={args.encoder_name} | ENCODER_WEIGHTS={args.encoder_weights}")
    ...
    for epoch in range(args.epochs):
        ...
        for imgs, masks in pbar:
            ...
            logits = forward_logits(model, imgs, args)
    ...
    print(f"Epoch checkpoints dir: {ckpt_dir}")
...
# =========================
def build_parser():
    p = argparse.ArgumentParser(description="Fireline DeepLabV3+ Training (RGB baseline + val metrics + TensorBoard)")
    ...
    p.add_argument("--encoder_name", type=str, default="resnet50",
                   help="DeepLabV3+ encoder (e.g., resnet50, resnet101)")
    ...
    p.add_argument("--run_name", type=str, default="deeplabv3plus_run", help="Run prefix")
...
#   --run_name deeplabv3plus_r50_dice_lr1e5